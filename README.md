# SysAdmin 🖖🏻 [![Progetto Wikitrek](https://img.shields.io/badge/Progetto-WikiTrek-996633?logo=data:image/svg%2bxml;base64,)](https://wikitrek.org/wiki/Wikitrek:Progetto)
Files riguardanti l'amministrazione del sistema

## `cloud-init`
Configurazioni per la preparazione automatica dei server usando [`cloud-init`](https://cloudinit.readthedocs.io/).

## `scripts`
Script per effettuare backup e aggiornamenti automatici.